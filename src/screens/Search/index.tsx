...
import {
  ...
  RemoveFavoriteList,
  getRecipeAction,
  getSearchRecipeAction,
  selectRecipeData,
  selectSavedRecipeData,
  selectSearchRecipeData,
...
const SearchScreen = ({route}) => {
  ...
  const favoriteData = useAppSelector(selectSavedRecipeData);
  const searchData = useAppSelector(selectSearchRecipeData(route.params.value));
  const getData = (number: number) => {
    dispatch(
      getSearchRecipeAction({
        number,
        query: route.params.value,
      }),
    );
  };
  useEffect(() => {
  ...
  const toggleFavorite = useCallback((item, isFavorite) => {
    if (isFavorite) {
  ...
  const renderFoodItems = useCallback(
    ({item, index}) => {
      const isFavorite = favoriteData.data.some(e => e.id === item.id);
      return (
  ...
  const loadmoreData = useCallback(() => {
    const currentNumber = searchData?.number ?? 10;
    console.log(currentNumber, 'haha');
    if (currentNumber >= 10) {
      getData(currentNumber + 10);
    }
  }, [searchData]);
  ...
  return (
    <Container>
      <HeaderContainer>
        <BackButtonTouchable onPress={() => goBack()}>
          <Icon name={'angle-left'} size={32} color={'black'} />
        </BackButtonTouchable>

        <ScreenTitle>Search Recipes</ScreenTitle>
      </HeaderContainer>
